...
## `offlineExportPin` — gate offline do export local (HUB-88)

| Campo | Tipo | Obrigatório | Validação |
|-------|------|-------------|-----------|
| `offlineExportPin` | `string` | Sim | 4 a 6 dígitos numéricos |

Validado com a regex `/^\d{4,6}$/`.

> **Importante (HUB-88 / ADR-012):** este campo **não** é o segredo do Admin online. Ele é
> um gate **apenas de UI**, de baixo valor, que destrava o export dos leads locais do
> IndexedDB **quando o totem está offline** (`navigator.onLine === false`). Deve ser um valor
> **distinto e sem relação** com a senha online.
>
> A **senha do Admin online** (que autoriza a leitura remota dos leads via RPC
> `admin_list_leads`) **não fica no `config.json`** — ela é verificada no servidor (Supabase,
> hash bcrypt) e nunca aparece em nenhum artefato baixado pelo cliente. Configure-a semeando o
> hash no SQL Editor — ver `docs/services-checklist.md`, seção "Segurança de leitura do Admin".
>
> O antigo `adminPin` (ex.: `3314`) foi **removido** e está **queimado** (era público via
> `GET /config.json`); não reutilize esse valor.

**Segurança:** o `offlineExportPin` protege o painel de operadores casuais offline, não é um
mecanismo robusto — os dados que ele protege já estão fisicamente no dispositivo. A proteção
real do dado remoto é a senha verificada no servidor.
...
Após 3 tentativas incorretas, a entrada do painel é bloqueada por 60 segundos.
...
| `offlineExportPin` | Ausente ou não corresponde a `/^\d{4,6}$/` |
